...
public class StickerSet {
  ...
  public string sticker_dir() {
    return( Path.build_filename( Environment.get_user_data_dir(), "annotator", "stickers" ) );
  }
  ...
  private bool create_custom_dir() {
    ...
    /* Add the README */
    var src = File.new_for_uri( "resource:///com/github/phase1geo/annotator/images/README.md" );
  ...
  public static Gdk.Pixbuf? make_pixbuf( string resource, int width = -1 ) {
    try {
      if( resource.get_char( 0 ) == '/' ) {
        ...
        return( new Pixbuf.from_resource_at_scale( ("/com/github/phase1geo/annotator/images/sticker_" + resource), ((width == -1) ? 64 : width), -1, true ) );
      }
    } catch( Error e ) {
      return( null );
    }
